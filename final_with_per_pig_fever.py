...
# ================= PER-PIG RELATIVE FEVER LOGIC (ADDED) =================
FEVER_DELTA = 2.0                 # °C above pig baseline
BASELINE_MIN_SAMPLES = 10
THERMAL_HISTORY_LEN = 30

pig_thermal_history = {}   # pid -> recent thermal values
pig_baseline = {}          # pid -> baseline temp
pig_max_seen = {}          # pid -> max observed
pig_current_temp = {}      # pid -> last temp

def extract_pig_temp(thermal_data, pig_box, cam_shape):
    Hc, Wc = cam_shape[:2]
    Ht, Wt = thermal_data.shape
    x1, y1, x2, y2 = pig_box
    ...
    tx2, ty2 = min(Wt, tx2), min(Ht, ty2)

    roi = thermal_data[ty1:ty2, tx1:tx2]
    if roi.size < 5:
        return None

    return float(np.percentile(roi, 95))

def update_baseline(pid):
    hist = pig_thermal_history.get(pid, [])
    if len(hist) < BASELINE_MIN_SAMPLES:
        return None
    baseline = float(np.percentile(hist, 50))
    pig_baseline[pid] = baseline
    return baseline
# ================= CONFIG =================
INPUT_SIZE = 416
SMALL_W = 320
