...
# --- GLOBAL (add once) ---
thermal_ema = None
THERMAL_ALPHA = 0.2  # temporal smoothing (0.1Ã¯Â¿Â½0.3)
...
BEHAVIOR_MODEL = "/home/asfrotect/Projects/BYMS_TFLITE_3_COMBINED/LucbanBehaviorV3_int8.tflite"
...
MAX_PIG_IDS = 12         # realistic max pigs per pen
...
# ================= FEVER LOGIC =================
FEVER_DELTA_C = 2.0          # >= 2Ã¯Â¿Â½C rise indicates fever
thermal_hourly_avg = {}     # hour_key -> avgTemp

# ================= GSM =================
GSM_PORT = "/dev/ttyAMA0"
GSM_BAUD = 9600
SMS_NUMBER = "+639082079549"
...
while True:
    ...
    if now >= next_detection_time:
        ...
        if len(prev_hours) >= 2:
            ...
            if delta >= FEVER_DELTA_C:
                fever_detected = True
                print(f"[FEVER] Temp increased by {delta:.2f}Ã¯Â¿Â½C")
        ...
        if alert_score > 0:
            ...
            # ?? LEVEL 1: SINGLE PARAMETER ALERTS
                ...
                if fever_detected or persistent_fever:
                    send_sms(
                        f"FEVER ALERT!\n"
                        f"Max Temp: {THERMAL_MAX_TEMP:.1f} C\n"
