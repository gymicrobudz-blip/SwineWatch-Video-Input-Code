...
# ================= CONFIG =================
IMG_SIZE = 256        # smaller for behavior
SKIN_SIZE = 320
SMALL_W = 320         # pig detector input
...
BEHAVIOR_INTERVAL = 6   # slower
SKIN_INTERVAL = 5       # slower
...
while True:
    ...
    pig_itp.set_tensor(pig_in[0]['index'], preprocess(small, IMG_SIZE))
    pig_itp.invoke()
    ...
    # ---- BEHAVIOR (ONLY BEST PIG) ----
    if pigs and frame_id % BEHAVIOR_INTERVAL == 0:
        (x1, y1, x2, y2), _ = pigs[0]
        roi = frame[y1:y2, x1:x2]
        if roi.size:
            beh_itp.set_tensor(beh_in[0]['index'], preprocess(roi, IMG_SIZE))
            beh_itp.invoke()
    ...
    if pigs and frame_id % SKIN_INTERVAL == 0:
        for (x1, y1, x2, y2), _ in pigs:
            ...
            skin_itp.set_tensor(
                skin_in[0]['index'], preprocess(roi, SKIN_SIZE)
            )
            skin_itp.invoke()
